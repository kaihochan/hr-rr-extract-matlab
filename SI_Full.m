myFiles = dir(fullfile('./Data/SI','*.xlsx'));
disp('Reading all excel files in ./Data/SI');
...
for i = 1:length(myFiles)
    ...
    loadPVDF = readtable(append('./Data/SI/',baseFileName),'Range','B:B');
    PVDFdata = table2array(loadPVDF);
    senRaw = PVDFdata(1:end,1);

    % calculate HR and RR based on readback data
    [HR,RR,hrFlt,rrFlt] = SI_Extraction(senRaw);
...
function bruteforceFindHRFreq
    ...
    loadPVDF = readtable('./Data/SI/Sit_with_cushion_Test_1_John.xlsx','Range','B:C');
    PVDFdata = table2array(loadPVDF);
    senRaw = PVDFdata(1:end,1);
...
function bruteforceFindRRFreq
    ...
    [refSound, refFs] = audioread('./Data/SI/Sit_without_anything_Test_4_John.m4a');
    
    % load data from excel file
    % col B is sensor data
    % col C is reference data
    loadPVDF = readtable('./Data/SI/Sit_without_anything_Test_4_John.xlsx','Range','B:C');
    PVDFdata = table2array(loadPVDF);
    senRaw = PVDFdata(1:end,1);
